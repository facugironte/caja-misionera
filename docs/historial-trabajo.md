...
## Precio "por pares" dinámico para Choripán/Hamburguesa/Veggie (2026-09-01)

Reemplaza el catálogo de 9 combos fijos del Buffet (Choripán + bebida, 2 choripán + bebida, Choripán + hamburguesa + bebida, etc.) por un motor de precio dinámico — pedido explícitamente porque mantener una combinación fija por cada cruce posible no escalaba (y fue lo que rompió la simetría de las tarjetas en el commit anterior).

- **Los 3 "principales"** (Choripán, Hamburguesa, Hamburguesa veggie) pasan a ser botones sueltos, **sin precio propio** (`product(..., null, { pairGroup: "principales" })`). Bebida y "Pizzeta + bebida (menores de 7)" (renombrado, antes "Bebida + pizzeta") siguen como productos normales con precio fijo.
- **Regla de precio** (`PAIR_GROUPS.principales` en `data.js`): las unidades de esos 3 productos que haya en el carrito (mezclando tipos libremente) se agrupan de a pares consecutivos — en orden fijo Choripán → Hamburguesa → Veggie, así los pares del mismo tipo se arman primero y solo se mezclan en el borde si hace falta. Cada par cuesta $18.000 (bebida incluida); si sobra una unidad suelta, esa va a $10.000 (bebida incluida). 1 → $10.000 · 2 → $18.000 · 3 → $28.000 (par + suelto) · 4 → $36.000 (2 pares), tal cual se pidió.
- **Sheets:** cada par/individual formado genera **una fila propia en DetalleVentas** (no una por ingrediente) con nombre autogenerado ("Choripán + hamburguesa + bebida", "2 choripán + bebida", etc. — misma convención de nombres que los combos fijos de antes), `cantidad`, `precio_unitario` y `subtotal` exactos — nada de precios prorrateados. En MovimientosStock cada par/individual sigue generando una fila por cada ingrediente que usó + una de Bebida, igual que los combos fijos de antes. Ejemplo verificado (2 hamburguesa + 1 choripán + 1 veggie = 4 principales): 2 filas en DetalleVentas ("Choripán + hamburguesa + bebida" $18.000, "Hamburguesa + hamburguesa veggie + bebida" $18.000) y en MovimientosStock: Choripán −1, Hamburguesa −2, Hamburguesa veggie −1, Bebida −2.
- El control de stock por pool (`pool.controlled`) sigue aplicando igual: si se apaga "Hamburguesa", los pares que la incluyen no descuentan ni se limitan por ella (pero sí por el otro ingrediente del par y por la bebida) — el precio no cambia.
- En Vender, los 3 principales se muestran con sus steppers normales pero sin $ por línea (el precio no es por-ítem); debajo aparece un bloque "Combo principales (bebida incluida)" con cada combinación ya armada y su importe — eso es lo que se cobra y lo que se confirma antes de "Vender".
- El grid de Buffet baja de 14 a 8 tarjetas (Choripán, Hamburguesa, Hamburguesa veggie, Bebida, Pizzeta + bebida, Café, Té, Torta), resolviendo de paso la asimetría/superposición reportada.
- Fix aparte: en la pantalla Stock, el nombre de un producto largo se superponía con el precio (`.set-row .name-stock .name` tenía `flex:none`, que le impedía hacer *wrap*) — se corrigió a `flex:1 1 100%; min-width:0`.
